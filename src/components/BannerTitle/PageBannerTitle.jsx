      ...
      (entries) => {
        if (entries[0].isIntersecting) {
          gsap.fromTo(
            ...
            { opacity: 0, x: 30 },
            { opacity: 1, x: 0, duration: 2, stagger: 0.5, delay: 0.2 }
          );
        }
      },
      { threshold: 0.5 }
    ...
    <section>
      <div className="flex flex-col md:flex-row gap-5 md:gap-2 justify-between md:items-center border-b-2 border-[#0fcda185] pb-3">
        <div>
          <h1
            ref={animatedTextRef}
            className="text-white text-xl md:text-2xl lg:text-4xl tracking-wider font-semibold"
          >
            {title}
          </h1>
          ...
          <Link to={btnLink} className="shrink-0">
            <button className="inline-flex items-center gap-5 px-5 py-3 group rounded-xl bg-gradient-to-tr from-[#1e2d4056] to-[#0fcda122] border border-gray-500 hover:border-[#0fcda1] focus:border focus:border-[#0fcda1] text-white">
              ...
              <span className="flex flex-col gap-1 items-start">
                <span className="text-xs text-slate-300">{btnText1}</span>
                <span className="font-semibold title-font">{btnText2}</span>
              </span>
            </button>
          </Link>
        )}
